...
class XmlFileHandler implements ServiceLocatorAwareInterface
{
    ...
    public function loadFile($tmpFileName)
    {
        ...
        if (! $success) {
            ...
            $this->cleanInfoBlocks();
            $this->fileIsLoaded = true;
            $this->fileIsValid = false;
        }
    }

    private function cleanInfoBlocks()
    {
        $infoBlocks = $this->cdmFrDomDocument->getElementsByTagName('infoBlock');
        foreach ($infoBlocks as $infoBlock) {
            $nodeValue = $infoBlock->nodeValue;
            $infoBlock->nodeValue = '';
            $infoBlock->appendChild($this->cdmFrDomDocument->createTextNode(strip_tags($nodeValue)));
        }
    }
    ...
    public function validateFile()
    {
        ...
        // TODO remettre la validation XSD quand Lheo aura remis son schéma en ligne
        // if (! $this->cdmFrDomDocument->schemaValidate(__DIR__ . '/../../../../xsd/CDMFR.xsd')) {
        if (false && ! $this->cdmFrDomDocument->schemaValidate(__DIR__ . '/../../../../xsd/CDMFR.xsd')) {
            $this->fileIsValid = false;
    ...
    public function getCdmFrObject()
    {
        if (! isset($this->cdmFrObject)) {
            ...
            $this->cdmObject = $analyzer->loadCdmFrObjectFromDocument($this->cdmFrDomDocument);
        }
        $this->cdmFrDomDocument = null;
